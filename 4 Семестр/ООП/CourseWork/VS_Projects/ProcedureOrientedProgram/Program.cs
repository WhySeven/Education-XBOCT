...
namespace ProcedureOrientedProgram {
    class Program {
        /* В методе Main вызываются методы DrawMenu и ReadInputData, обеспечивающие работу меню, 
         * а также алгоритмы действий при выборе определённых пунктов меню*/
        static void Main() {
        ...
        // Метод MonteCarlo, осуществляющего расчет данных и вывод их в таблицу
        public static void MonteCarlo(double ax, double ay, double ex, double ey, double gx, double gy) {
            ...
            double dx = ex + (ey - ay);
            double dy = ay;
            // Cписок в который будет записываться площадь, вычисленная методом Монте-Карло
            List<double> monte_carlo_Square = new List<double>();
            /* Cписок в который будет записываться относительная ошибка
             * вычислений каждой итерации метода Монте-Карло*/
            List<double> relError = new List<double>();
            // Cписок в который будет записываться время затраченное на вычисления
            List<long> ms = new List<long>();
            // Поле, которе будет отвечать за кол-во итераций цикла, описанного ниже
            int step = 0;
            // Поле в которое будет записываться точная площадь фигуры
            ...
            for (int N = Convert.ToInt32(Math.Pow(10, 3)); N <= Math.Pow(10, 7); N *= 10) {
                ...
                monte_carlo_Square.Add(MonteCarloSquare(dx, dy, ex, ey, gx, gy, ox, oy, rectangle_Square, N));
        ...
        // Метод возвращающий точную площадь фигуры
        ...
        static double MonteCarloSquare(double dx, double dy, double ex, double ey, double gx, double gy, double ox, double oy, double rectangle_Square, int N) {
            Random random = new Random();
            int count = 0;
            for (int i = 0; i < N; i++) {
                double x = random.NextDouble() * (dx-gx) + gx;
                double y = random.NextDouble() * (ey-dy) + dy;
        ...
        // 2 метода для вывода таблицы в консоль
        ...
        // Метод возвращающий список ошибок
        ...
        // Индекс пункта меню на котором находится пользователь
        static int currentMenuIndex = 0;
        // Метод, отвечающий за то, как выглядит меню
        ...
        // Метод, отвечающий за управление меню
